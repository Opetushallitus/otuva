...
function main {
  ...
  if [ "${ENV}" = "hahtuva" ]; then
    export TUNNEL_PORT="6691"
  elif [ "${ENV}" = "dev" ]; then
    export TUNNEL_PORT="6692"
  elif [ "${ENV}" = "qa" ]; then
    export TUNNEL_PORT="6693"
  elif [ "${ENV}" = "prod" ]; then
    export TUNNEL_PORT="6694"
  fi

  export IMAGE_TAG="cas-oppija-psql-tunnel"
  export DB_SECRET="OtuvaCasOppijaDatabaseSecret"
