...
task_arn="$( aws ecs list-tasks --cluster "${cluster_arn}" --service-name "${SERVICE_NAME}" --query 'taskArns[0]' --output text )"
container_id=$( aws ecs list-tasks --cluster "${cluster_arn}" --service-name "${SERVICE_NAME}" --query 'taskArns[0]' --output text | cut -d '/' -f 3 )
container_runtime_id="$( aws ecs describe-tasks --cluster "${cluster_arn}" --tasks "${task_arn}" --query "tasks[0].containers[?name == 'BastionContainer'] | [0].runtimeId" --output text )"
