...
function deploy_env {
  ...
  npx cdk --app "npx ts-node ${repo}/infra/src/cdk-app.ts" deploy --require-approval never --all
}
...
function bootstrap_cdk_for_env {
  ...
  if [ "${env}" == "util" ]; then
      npx cdk bootstrap "aws://${account_id}/${region}" \
          --qualifier kayttooike --toolkit-stack-name KayttooikeusCDKToolkit
  else
      ...
      npx cdk bootstrap "aws://${account_id}/${region}" \
          ...
          --cloudformation-execution-policies "arn:aws:iam::${account_id}:policy/KayttooikeusCDKDeploymentTargetPermissions"
...
function setup_cdk_deployment_target_policy {
  npx ts-node "${repo}/infra/src/setup-cdk-deployment-target-policy.ts"
}
