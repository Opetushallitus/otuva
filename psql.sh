...
IMAGE_TAG="yleiskayttoiset-psql-tunnel"

function main {
  export ENV="$1"
  export_aws_credentials "$ENV"
  if [ "${ENV}" = "hahtuva" ]; then
    export TUNNEL_PORT="6671"
  elif [ "${ENV}" = "dev" ]; then
    export TUNNEL_PORT="6672"
  elif [ "${ENV}" = "qa" ]; then
    export TUNNEL_PORT="6673"
  elif [ "${ENV}" = "prod" ]; then
    export TUNNEL_PORT="6674"
  fi
...
function start_tunnel {
  ...
  container_id=$( docker run \
    --env ECS_CLUSTER_NAME --env DB_SECRET \
