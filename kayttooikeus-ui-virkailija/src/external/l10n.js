...
const l10nResponseS = l10nRequestS.flatMap(url => Bacon.fromPromise(fetch(url).then(handleError).then(response =>  response.json())));
const lokalisointiRequestS = Bacon.combineWith(urlsP, urls => urls.url('lokalisointi.localisation')).toEventStream();
const lokalisointiResponseS = lokalisointiRequestS.flatMap(url => Bacon.fromPromise(fetch(url).then(handleError).then(response =>  response.json())));
